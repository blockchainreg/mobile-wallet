...
export default ({ store, web3t }) => {
  ...
  const done = () => {
    if (store.signUpConfirmSeedField == '')
      return Toast.show({ text: lang.emptyWord });

    if (
      store.current.seedWords.length > 1 &&
      bip39.wordlists.EN.indexOf(store.signUpConfirmSeedField) === -1
    ) {
    ...
    try {
      for (let i = 0; i < store.current.seedWords.length - 11; i += 12) {
        bip39.mnemonicToEntropy(
          store.current.seedWords.slice(i, i + 12).join(' ')
        );
      }
