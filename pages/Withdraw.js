...
export default ({ store, web3t }) => {
  ...
  const scanQRSend = () => {
	...
	const btnWithdraw = ({ store, web3t }) => {
	  ...
	  console.log("send.error.length", send.error.length)
    console.log("send.amountChanging === true", send.amountChanging === true)
    console.log("store.current.creatingTransaction", store.current.creatingTransaction)
