...
import { Text, View, Item, Button, Icon, Toast } from 'native-base';
import {
  Image,
  ImageBackground,
      ...
      return (
        <>
          ...
          <Text style={styles.textAuth}>
            Please register at least one Fingerprint or Face ID in the setting
            of your Smartphone to use this feature.
          </Text>
        </>
      );
    case 'waiting':
...
function RequestPin({ store, web3t }) {
    ...
    const login = () => {
      setTimeout(async () => {
        if (!check(pin)) {
          setPin('');
          return Toast.show({
            text: lang.incorrectPass || 'Incorrect password',
          });
        }
  ...
  const buttonInactive = (store) => {
    return (
      <Button block style={styles.buttonInactive}>
        <Text style={styles.buttonTextInactive}>
          {capitalize(lang.confirm)}
        </Text>
      </Button>
    );
  };
  ...
  const checkpin = (store) => {
    ...
    const validInputPin = !pin || regexPin.test(pin);
    // Validation end

    return (
      <View style={styles.marginBtn}>
        {pin && validInputPin ? buttonActive(store) : buttonInactive(store)}
        <View height={15}></View>
      </View>
    );
  };
...
export default ({ store, web3t }) => (
  <LocalAuthenticationEnable store={store} web3t={web3t} />
);
