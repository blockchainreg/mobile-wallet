...
export default ({ store, web3t, props }) => {
  ...
  const TOTAL_STAKE = !details.myStake.isZero()
    ? formatStakeAmount(details.myStake)
    : formatStakeAmount(details.activeStake);
  ...
  const onPressButton = () => {
    if (
      ...
      (parseFloat(store.amount) &&
        amountToBN(store.amount).gte(AVAILABLE_BALANCE.sub(new BN(1e9))))
    )
      return null;
