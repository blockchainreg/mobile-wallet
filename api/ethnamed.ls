require! {
    ...
    \../web3t/providers/superagent.js : { put, get, post }
    \solidity-sha3 : { sha3num }
    \./config.json : { url }
    \./get-verify-email-url.js
    \./json-parse.js
}
to-hex = (o)->
    return o if not o?
...
get-access-key = ({ name, record }, cb)->
    ...
    err, data <- put "#{url}/naming" , { domain, record } .end
    return cb data.text if err?
    json-parse data.text, cb
...
builder-setup-record = (web3, contract)-> ({ amount-ethers, name, record, owner }, cb)->
    ...
    return cb null, access-key.result if access-key.result?
...
builder-whois = (web3, contract)-> (record, cb)->
    hash = sha3num record
    err, data <- get "#{url}/naming/whois/#{hash}" .end
    return cb data.text if err?
    cb null, data.text
...
#override centralized 
builder-verify-record = (web3, contract) -> (name, cb)->
    err, data <- post "#{url}/naming/resolve", { name } .end
    return cb data.text if err?
    cb null, data.text
