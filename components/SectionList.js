...
export default observer( ({ store, web3t, ...props }) => {
	...
	const deleteItem = (rowMap, data) => {
		...
		spin(store, `${lang.uninstalling || "Uninstalling"} ${tokenName}`, web3t.uninstall.bind(web3t))(coin.token, (err, data) => {
		});
	};
	...
	const renderHiddenItem = (data, rowMap) => {
		if (['vlx_evm', 'vlx_native', 'vlx2', 'btc', 'eth'].indexOf(data.item.coin.token) > -1) {
			return null;
		}
