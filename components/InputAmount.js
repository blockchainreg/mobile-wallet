...
export default class InputAmount extends Component {
  ...
  static getFormattedValue(value, maxFractionLength = 3) {
    ...
    while (
      int.split('|').join('').length > 1 &&
      (int[0] === '0' || int[0] === ',')
    )
      int = int.slice(1);
    fraction = Array.from(fraction)
      .filter((c) => c === '.' || isDigit(c))
      .join('')
      .slice(0, maxFractionLength);
    return int + fraction;
  }
  ...
  handleTextChange = (text) => {
    ...
    const text2 = [
      text.slice(0, this.state.selection.end + 1),
      '|',
      text.slice(this.state.selection.end + 1),
    ].join('');
    const formatted = InputAmount.getFormattedValue(
      text2,
      this.props.maxFractionLength
    );
  ...
  static getDerivedStateFromProps(props, state) {
    if (state.value !== props.value) {
      ...
      return {
        value: InputAmount.getFormattedValue(
          props.value,
          props.maxFractionLength
        ),
      };
    }
    return null;
