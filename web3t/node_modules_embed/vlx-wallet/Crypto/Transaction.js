...
var Transaction = /** @class */ (function (_super) {
  ...
  function Transaction(
    ...
    _this.toJSON = function () {
      var tx = __assign(
        __assign({}, _.pick(_this, ['hash', 'version', 'lock_time'])),
        {
          tx_out: _this.tx_out.map(function (txOut) {
            return __assign(
              __assign(
                {},
                _.pick(txOut, [
          ...
          tx_in: _this.tx_in.map(function (txIn) {
            return __assign(__assign({}, txIn), {
              public_key: Buffer.from(txIn.public_key, 'hex').toString(
                'base64'
              ),
              previous_output: __assign(__assign({}, txIn.previous_output), {
    ...
    _this.generateHash = function () {
      ...
      var payloadIn = _this.tx_in.reduce(function (res, txIn) {
        ...
        p2 = Buffer.concat([
          p2,
          utils_1.BigIntToBufferLE(txIn.previous_output.value, 8),
        ]);
    ...
    _this.hashOuts = function () {
      return _this.tx_out.reduce(function (res, out) {
        return Buffer.concat([res, Transaction.hashOut(out)]);
      }, Buffer.alloc(0));
    };
    var totalin = unspents.reduce(function (res, po) {
      return res.add(po.value);
    }, bigInt(0));
    var index = 0;
    _this.tx_out = [];
