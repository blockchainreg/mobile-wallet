...
export default class Transaction extends TransactionBase<BigInteger> {
  constructor(
    sodium,
    unspents: Out<BigInteger>[],
    amount: BigInteger,
    velasKey: HDKeys,
    ...
    super();
    const totalin = unspents.reduce((res, po) => res.add(po.value), bigInt(0));

    let index = 0;

    this.tx_out = [];

    const txOut1 = new TxOut<BigInteger>();
    txOut1.index = index++;
    txOut1.pk_script = '';
    txOut1.value = commission;
    this.tx_out = [...this.tx_out, txOut1];

    const txOut2 = new TxOut<BigInteger>();
    txOut2.index = index++;
    ...
    this.tx_in = unspents.map((po: Out<BigInteger>): TxIn<BigInteger> => {
    ...
    const tx: TransactionDto = {
      ...
      tx_in: this.tx_in.map((txIn) => ({
        ...
        previous_output: {
          ...
          value: Number(txIn.previous_output.value),
        },
      })),
    };
  ...
  private hashOuts = (): Buffer => {
    return this.tx_out.reduce(
      (res, out) => Buffer.concat([res, Transaction.hashOut(out)]),
      Buffer.alloc(0)
    );
  };
  ...
  private static hashOut = (val: TxOut<BigInteger>): Buffer => {
    let buffer = Buffer.alloc(4);
    buffer.writeInt8(val.index, 0);
    const value = BigIntToBufferLE(val.value, 8);
