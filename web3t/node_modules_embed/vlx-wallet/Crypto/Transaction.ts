...
export default class Transaction extends TransactionBase<BigInteger> {
	constructor(sodium, unspents: Out<BigInteger>[], amount: BigInteger, velasKey: HDKeys, changeAddress: string, to: string, commission: BigInteger) {
		super();
	...
	private generateHash = (): string => {
		...
		const payloadIn = this.tx_in.reduce((res: Buffer, txIn: TxIn<BigInteger>) => {
			const p1 = Buffer.from(txIn.previous_output.hash, 'hex');
			let p2 = Buffer.alloc(4);
			p2.writeUInt32LE(txIn.previous_output.index, 0);
			p2 = Buffer.concat([p2, BigIntToBufferLE(txIn.previous_output.value, 8)]);
