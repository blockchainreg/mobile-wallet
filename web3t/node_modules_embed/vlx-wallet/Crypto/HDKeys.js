...
exports.CKDPriv = function (_a, index) {
  ...
  var I = crypto_1.createHmac('sha512', chainCode).update(data).digest();
  var IL = I.slice(0, 32);
  var IR = I.slice(32);
  return {
    key: IL,
...
exports.derivePath = function (path, seed) {
  ...
  var _a = exports.getMasterKeyFromSeed(seed),
    key = _a.key,
    chainCode = _a.chainCode;
  var segments = path
    .split('/')
  ...
  return segments.reduce(
    function (parentKeys, segment) {
      return exports.CKDPriv(parentKeys, segment + HARDENED_OFFSET);
    },
...
exports.isValidPath = function (path) {
  ...
  return !path.split('/').slice(1).map(exports.replaceDerive).some(isNaN);
};
exports.pathRegex = new RegExp("^m(\\/[0-9]+')+$");
exports.replaceDerive = function (val) {
  return val.replace("'", '');
};
