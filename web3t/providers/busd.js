...
(function(){
  ...
  transformTx = curry$(function(network, description, t){
    var url, FOREIGN_BRIDGE, dec, tx, amount, time, gasUsed, gasPrice, txType, fee, recipientType, ref$, res;
    url = network.api.url;
        ...
        return calcGasPrice({
          ...
          return toEthAddress(account.address, function(err, address){
            ...
            return getBalance({
              ...
              web3 = getWeb3(network);
              return web3.eth.getBalance(account.address, function(err, bnbBalance){
                var bnbBalanceEth;
                if (err != null) {
                  return cb(err);
                }
                bnbBalanceEth = toEth(bnbBalance);
                if (+bnbBalanceEth < +amountFee) {
                  return cb("BNB balance is not enough to send tx");
                }
                return getGasEstimate({
                  network: network,
                  feeType: feeType,
                  ...
                  return makeQuery(network, 'eth_chainId', [], function(err, chainId){
                    ...
                    return makeQuery(network, 'net_version', [], function(err, networkId){
                      var common, gasPrice, web3, contract, $data, to, onePercent, $gasEstimate, res, txObj, tx, rawtx;
                      if (err != null) {
                        return cb(err);
                      }
  ...
  getContractInstance = function(web3, network){
    var abi;
