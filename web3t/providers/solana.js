...
(function(){
	...
	out$.createTransaction = createTransaction = function(config, cb){
		...
		return getRecentBlockhash(network, function(err, recentBlockhash){
			...
			if (swap != null && swap === true) {
				...
				/* If it is swap config.data contains transaction object prepared for swap */
				transaction = config.data || {};  // transaction should be defined anyway and it cannot be null
	...
	prepareTxs = function(network, arg$, address, cb){
		...
		return makeQuery(network, 'getConfirmedTransaction', [signature, 'jsonParsed'], function(err, data){
			var t, txData, sender, receiver, hash, amount, ref$, fee, status, transaction, accountKeys, instructions, type, ref1$, ref2$, senders, receiverIndex, receivers, e, time, dec, url, cluster, customUrl, $cluster, uri, _type, recipientType, isStake, ref3$, ref4$, txType, _tx;
			if (err != null) {
