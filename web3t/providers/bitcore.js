...
(function(){
  var moment, ref$, map, foldl, any, each, find, sum, filter, head, values, join, reverse, get, post, plus, minus, div, times, BitcoinLib, bip39, jsonParse, deadline, decode, segwitAddress, segwitAddress2, getBitcoinFullpairByIndex, findMax, getEnough, calcFeePerByte, calcDynamicFee, getCalcFeeFunc, calcFeePrivate, calcFeeInstantx, calcFee, getKeys, extend, getDec, addAmount, getOutputs, parseRateString, extractVal, parseResult, getDepositAddressInfo, getDepositAddressFromList, getDepositAddress, addOutputsPrivate, addOutputs, getError, createTransaction, pushTx, getTotalReceived, getUnconfirmedBalance, getBalance, transformIn, transformOut, isValidAddress, transformTx, getApiUrl, checkTxStatus, getTransactions, slice$ = [].slice, toString$ = {}.toString, out$ = typeof exports != 'undefined' && exports || this;
  moment = require('moment');
  ref$ = require('prelude-ls'), map = ref$.map, foldl = ref$.foldl, any = ref$.any, each = ref$.each, find = ref$.find, sum = ref$.sum, filter = ref$.filter, head = ref$.head, values = ref$.values, join = ref$.join, reverse = ref$.reverse;
  ...
  out$.getTransactions = getTransactions = function(arg$, cb){
	...
	return get(getApiUrl(network) + "/address/" + address + "/txs?limit=50").timeout({
	  deadline: 15000
	}).end(function(err, data){
	  if (err != null) {
		return cb(err);
  ...
  var prepareRawTxs, prepareTxs, slice$ = [].slice;
  ...
  prepareTxs = function(result, network, arg$, cb){
	var tx, rest, hash;
	...
	hash = tx.mintTxid;
	return get(getApiUrl(network) + "/tx/" + hash + "/coins").timeout({
	  deadline: 15000
	}).end(function(err, data){
	  if (err != null) {
		return cb(err);
	  }
	  return jsonParse(data.text, function(err, rawData){
		var inputs, outputs, _tx, t;
		if (err != null) {
		  return cb(err);
		}
		inputs = rawData.inputs, outputs = rawData.outputs;
		_tx = outputs[0];
		t = _tx != null
			? [_tx]
			: [];
		return prepareTxs(t, network, rest, function(err, other){
		  var all;
		  if (err != null) {
			return cb(err);
		  }
