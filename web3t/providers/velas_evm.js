...
(function(){
  var stringify, ref$, filter, map, foldl, each, sortBy, reverse, uniqueBy, plus, minus, times, div, fromHex, $toHex, get, post, Web3, Tx, BN, hdkey, bip39, jsonParse, deadline, decode, encode, Common, vlxToEth, ethToVlx, sha3, bignumber, isChecksumAddress, isAddress, toEthAddress, isValidAddress, getEthereumFullpairByIndex, tryParse, makeQuery, getTransactionInfo, getGasEstimate, calcFee, getKeys, round, toHex, up, transformTx, getInternalTransactions, getExternalTransactions, getTransactions, getDec, calcGasPrice, tryGetLatest, getNonce, createTransaction, checkDecodedData, pushTx, checkTxStatus, getTotalReceived, getUnconfirmedBalance, getBalance, getSyncStatus, getPeerCount, getMarketHistoryPrices, toString$ = {}.toString, out$ = typeof exports != 'undefined' && exports || this;
  ...
  out$.isValidAddress = isValidAddress = function(arg$, cb){
    ...
    if (!isValid) {
      return cb("Given address is not valid Velas EVM address");
    }
    return cb(null, true);
  };
  ...
  transformTx = curry$(function(network, description, t){
    var url, HECO_SWAP__HOME_BRIDGE, BSC_SWAP__HOME_BRIDGE, dec, tx, amount, time, gasUsed, gasPrice, txType, fee, recipientType, ref$, res;
    url = network.api.url;
    HECO_SWAP__HOME_BRIDGE = network.HECO_SWAP__HOME_BRIDGE, BSC_SWAP__HOME_BRIDGE = network.BSC_SWAP__HOME_BRIDGE;
    dec = getDec(network);
    network = 'eth';
    tx = (function(){
      switch (false) {
    ...
    txType = (function(){
      switch (false) {
      case up(t.from) !== up('0x56454c41532d434841494e000000000053574150'):
        return "Native → EVM Swap";
      case up(t.to) !== up('0x56454c41532d434841494e000000000053574150'):
        return "EVM → Native Swap";
      case up(t.to) !== up(HECO_SWAP__HOME_BRIDGE):
        return "EVM → HECO Swap";
      case up(t.to) !== up(BSC_SWAP__HOME_BRIDGE):
        return "EVM → BSC Swap";
      default:
        return null;
      }
...
//      offset: offset
  ...
  out$.createTransaction = createTransaction = curry$(function(arg$, cb){
    var network, account, recipient, amount, amountFee, data, feeType, txType, gasPrice, gas, dec, privateKey;
    ...
    if (!isAddress(recipient)) {
      return cb("Address is not correct Velas EVM address");
    }
