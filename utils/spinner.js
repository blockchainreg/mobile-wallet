export default class Spinner {
  constructor(store, description, options = {}) {
    ...
    if (
      options.displayDescription === 'auto' ||
      options.displayDescription === undefined
    ) {
  ...
  finish() {
    console.log(
      this.description + ' took ' + (Date.now() - this.startedAt) + 'ms'
    );
    ...
    if (index === -1) {
      console.warn(
        'Spinner not found. It may already be disposed',
        this.description
      );
      return;
    }
    ...
    if (this.isDescriptionPushed) {
      const index = this.store.current.loadingDescriptions.indexOf(
        this.description
      );
      if (index === -1) {
        console.warn(
          'Spinner description not found. Something goes wrong.',
          this.description
        );
        return;
      }
