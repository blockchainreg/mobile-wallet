...
module.exports = function({ store, web3t}) {
	...
	var lang, wallets, chosenNetworkWallet, send, wallet, vlx_evm_to_vlx_erc20_swap, velas_evm_to_heco_swap, 
		...
		FOREIGN_BRIDGE, receiver, web3, contract, toHex, token,
		executeContractData, beforeSendAnyway, sendAnyway,  cancel, recipientChange, getValue, amountChange, 
		performAmountEurChange, performAmountUsdChange, amountEurChange, amountUsdChange, encodeDecode, showData, showLabel, 
		whenEmpty, history, network, invoice, name, ref$, feeToken, isData, bridgeFeeToken, chooseAuto, chooseCheap, 
		chooseCustom, chosenCheap, chosenAuto, chosenCustom, sendOptions, pending, calcAmountAndFee, flag, useMax, useMaxTryCatch, 
		useMaxAmount, hasSendError, homeFee, homeFeeUsd, getHomeFee;
	...
	*/
	usdc_to_usdc_velas_swap = function (token, chosenNetwork, cb) {
		var web3, ref$, FOREIGN_BRIDGE, FOREIGN_BRIDGE_TOKEN, value, receiver, contract;
		...
		return checkAllowedAmount({
			...
			var coin, gas, gasPrice, amountSend, amountSendFee, feeType, network, txType, web3, ref$, ref1$, ref2$, ref3$,
					contract, minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, data;
			if (err != null) {
				return cb(err);
			}
	...
	*/
	...
	busd_to_busd_velas_swap = function (token, chosenNetwork, cb) {
		var wallets, chosenNetworkWallet, ref$, FOREIGN_BRIDGE, FOREIGN_ERC20_TOKEN, web3, contract, abi, totalSupply, err,
				value, receiver, allowedRaw, allowed, coin, gas, gasPrice, amountSend, amountSendFee, feeType, network, txType,
				data, txObj;
		...
		ref$ = wallet.network, FOREIGN_BRIDGE = ref$.FOREIGN_BRIDGE, FOREIGN_ERC20_TOKEN = ref$.FOREIGN_ERC20_TOKEN;
		web3 = new Web3(new Web3.providers.HttpProvider(wallet.network.api.web3Provider));
		web3.eth.providerUrl = wallet.network.api.web3Provider;
	...
	*/
	...
	velas_evm_to_heco_swap = function (token, chosenNetwork, cb) {
		var web3, ref$, receiver, ref1$, ref2$, ref3$, contract, network,
		  minPerTxRaw, minPerTx, maxPerTxRaw, maxPerTx, data, HECO_SWAP__HOME_BRIDGE;
